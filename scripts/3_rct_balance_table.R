...
prepare_summary_column <- function(data.balance){
  ...
  tab <- data.balance %>%
    transmute(
      ...
      r04b_category_a_incidents_only = cndct_pretreat_guilty_rate_a
    )
...
tab_for_latex <- tab_for_latex[-14,] #drop empty row. HARD CODED
...
# # Merge with static characteristics from basic ####
# covariates <- c(#"est_days_served_on_20220501",
...
# 
# 
# data.balance <- left_join(randassign, basic[-which(duplicated(basic$research_id)),c("research_id", covariates)])
# 
# # Drop lifers
...
# # Randomization inference for pvalues ####
# # F-test on Balance test
# data.balance.reg <- data.balance[,c("treated", "stratum", covariates)]
