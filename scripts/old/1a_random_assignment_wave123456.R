...
randassign0 <- data.frame(treated = c(rep(1,7), rep(0,7)),
                          id_num = c("as0160", "bf6556", "bm7165", "ce6457", "cy7366",
                                     "as2609","bh6778",
                                     "af4296", "am8328", "ap1973", "be8100", "eg5999",
                                     "bj8105", "ay5858"), # lifers
                          stratum = "lifer",
                          treatment_wave = c(rep(0,6), 2.5, rep(0,6), 2.5),
                          treatment_date = c(rep(NA,6), "2023/4/4", rep(NA,6), "2023/4/4"))
...
# Release dates ####
# Using just the latest release date file which was updated ahead of wave 6
# Old release date code at the bottom of this script
# rel <- xl.read.file("data/raw/4_random_assignment/release_dates/240515_release_dates_from_MT_20240523_r5.xlsx", password = "LS2024", xl.sheet=1)
rel <- xl.read.file("data/raw/4_random_assignment/release_dates/2024_10_28_241015_release_dates_for_CHS_r6.xlsx", password = "LS2024", xl.sheet=1)
...
merged.file <- left_join(randassign, rel)
randassign <- left_join(randassign, rel)

# Mask IDs ####
names(randassign)[which(names(randassign)=="id_num")] <- "original_id"
randassign$original_id <- tolower(randassign$original_id)
i <- unique(randassign$original_id)
id.link <- mask_ids(i) # Generate masked IDs
randassign <- left_join(randassign, id.link, by="original_id") # Merge basic on original ID
randassign <- randassign[,-which(names(randassign)=="original_id")] # delete original ID
randassign <- randassign[,c(ncol(randassign),1:ncol(randassign)-1)] # reorder columns
