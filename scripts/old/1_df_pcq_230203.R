...
names.cols <- paste0("mcc_", scales.list$scale_theory)
pcq2[,names.cols] <- NA
...
for(i in scales.list$scale){
  ...
  col_mcc <- grep(paste0("mcc_",i), names(pcq2)[grep(i, names(pcq2))], value=TRUE)
  col_nqs <- grep(paste0("nqs_",i), names(pcq2)[grep(i, names(pcq2))], value=TRUE)
  pcq2[rows.without.nas,col_mcc] <- rowSums(pcq2[rows.without.nas,qnos$qno], na.rm=TRUE) #summing answered questions
  for(j in 1:nrow(pcq2)){
    ...
    if(answered_questions %in% c(0, NA)){
      ...
      pcq2[j,col_mcc] <- pcq2[j,col_mcc]/pcq2[j,col_nqs]
    }
  }
...
# Add same columns to pcq2 
start.col <- which(names(pcq2)=="mcc_prisoners")
end.col <- which(names(pcq2)=="sd_visits")
