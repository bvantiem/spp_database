...
## THINGS TO FIX BEFORE RE_RUNNING ANALYSIS ####
# Drop 12 individuals below who have more than 10 missing answers 
...
# length(which(complete.cases(pcq[,1:64])))
...
tab <- data.frame(question = pcq_lookup[pcq_lookup$question_qno %in% names(pcq[,1:64]),"question_pa_2022a"],
                  question_qno = pcq_lookup[pcq_lookup$question_qno %in% names(pcq[,1:64]),"question_qno"],
                  domain = pcq_lookup[pcq_lookup$question_qno %in% names(pcq[,1:64]),"domain_no_bosma"],
                  scale = pcq_lookup[pcq_lookup$question_qno %in% names(pcq[,1:64]),"scale_theory_long"],
                  missing = NA,
                  no_opinion = NA)
for(i in names(pcq[which(rowSums(is.na(pcq))<=10),1:64])){
  tab[tab$question_qno==i,"missing"] <- length(which(is.na(pcq[,i])))
  tab[tab$question_qno==i,"no_opinion"] <- length(which(pcq[,i]==111))
}
tab <- tab[order(tab$domain),c("question", "missing", "no_opinion")]
names(tab) <- c("Item","Missing", "No Opinion")
print(xtable(tab, digits=c(0,0,0,0), caption="Missingness by Item"),include.rownames=FALSE,
      file="output/tables/missingness.txt") 
...
parallel <- fa.parallel(pcq_am[,qnos], fm="ml", fa="both", sim=FALSE)
...
fa.ml.oblimin <- fa(r=pcq_am[,qnos],  # pcq2[,qnos]
                    ...
                    fm="ml", # ml for max likelihood (consider paf, used in Malandrone)
                    rotate="oblimin",
                    warnings=TRUE,
                    missing=TRUE,
