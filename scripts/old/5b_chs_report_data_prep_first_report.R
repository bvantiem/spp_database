...
i <- match(pcq_mf$unit_type, unit_mapping$unit_type)
pcq_mf$unit_type <- unit_mapping$unit_type_named[i]
...
f.likert.scale.by.unit.plot <- function(scale_x, plot.title){
  ...
  data_for_graph <- pcq_mf %>% filter(!is.na(unit_type), survey_wave==3) %>%
  ...
  p <- ggplot() + geom_bar(data=highs, aes(x = unit_type, y=value, fill=col), position="stack", stat="identity") +
    ...
    scale_fill_identity("Percent", labels = mylevels, breaks=pal, guide="legend") +
  ...
  ggsave(paste0("output/dedicated_analyses/chs_report/scale_plots/plot_", plot.title, ".jpg"), p, width = 8, height = 4, dpi = 300)
}
...
i <- match(pcq$unit_type, unit_mapping$unit_type)
pcq$unit_type <- unit_mapping$unit_type_named[i]
...
f.likert.item.by.unit.plot <- function(item_x, plot.title){
  ...
  p <- ggplot() + geom_bar(data=highs, aes(x = unit_type, y=value, fill=col), position="stack", stat="identity") +
    geom_bar(data=lows, aes(x = unit_type, y=-value, fill=col), position="stack", stat="identity") +
    geom_hline(yintercept = 0, color =c("white")) +
    scale_fill_identity("Percent", labels = mylevels, breaks=pal[-3], guide="legend") +
    theme_fivethirtyeight() +
    coord_flip() +
    labs(title=str_to_title(gsub("_", " ", plot.title)), y="",x="") +
    theme(plot.title = element_text(size=14, hjust=0.5)) +
  ...
  ggsave(paste0("output/dedicated_analyses/chs_report/item_plots/plot_", plot.title, ".jpg"), p, width = 8, height = 3.5, dpi = 300)
}
...
f.likert.item.by.unit.plot(q37, "there_are_a_lot_of_drugs_on_this_unit")
f.likert.item.by.unit.plot(q42, "staff_use_too_much_force_on_this_unit")
f.likert.item.by.unit.plot(q42, "i_am_satisfied_with_the_recreation_activities")
...
f.likert.item.by.unit.plot <- function(item_x, plot.title, plot.name){
  ...
  ggsave(paste0("output/dedicated_analyses/chs_report/item_plots/plot_", plot.name, ".jpg"), p, width = 8, height = 3.5, dpi = 300)
...
my.data <- pcq2 %>% filter(survey_wave==3)
...
ggsave("output/dedicated_analyses/chs_report/item_plots/plot_activities.jpg", p, width = 8, height = 3.5, dpi = 300)
...
ggsave("output/dedicated_analyses/chs_report/item_plots/plot_care_services.jpg", p, width = 8, height = 3.5, dpi = 300)
...
ggsave("output/dedicated_analyses/chs_report/item_plots/plot_visits.jpg", p, width = 8, height = 3.5, dpi = 300)
...
ggsave("output/dedicated_analyses/chs_report/item_plots/plot_grievances.jpg", p, width = 8, height = 3.5, dpi = 300)
...
ggsave("output/dedicated_analyses/chs_report/item_plots/plot_food.jpg", p, width = 8, height = 3.5, dpi = 300)
...
ggsave("output/dedicated_analyses/chs_report/item_plots/plot_staff.jpg", p, width = 8, height = 3.5, dpi = 300)
