...
We aim to identify the underlying latent variables in the prison climate questionnaire. While the researchers have pre-existing ideas about what these underlying variables are, we decide to first use a data driven approach. We will use the data from the first survey wave as a 'training set', and impute missing data using a random forest approach. We will identify a 'data driven' factor structure through an exploratory factor analysis (EFA). EFA is a close cousin to Principal Components Analysis we have learnt in this class. We will then combine our theoretical knowledge with the results from the EFA to re-specify the factor structure. We will use data from 'new' individuals in the second survey wave as a 'test set' to obtain model fit indices in a confirmatory factor analysis. We will use the model from the EFA to identify if individuals appear to cluster by the unit on which they live. 
...
pcq <- pcq[,c(retain, "research_id", "unit_type")]
```
...
```{r, results='asis'}
# Missing data table
dat <- pcq %>% select(- unit_type, -research_id)
tab <- data.frame(question = pcq_lookup[pcq_lookup$question_qno %in% names(dat[,1:ncol(dat)]),"question_pa_2022a"],
                  question_qno = pcq_lookup[pcq_lookup$question_qno %in% names(dat[,1:ncol(dat)]),"question_qno"],
                  domain = pcq_lookup[pcq_lookup$question_qno %in% names(dat[,1:ncol(dat)]),"domain_no_bosma"],
                  scale = pcq_lookup[pcq_lookup$question_qno %in% names(dat[,1:ncol(dat)]),"scale_theory_long"],
                  missing = NA,
                  no_opinion = NA)
for(i in names(dat[which(rowSums(is.na(dat))<=10),1:ncol(dat)])){
  tab[tab$question_qno==i,"missing"] <- length(which(is.na(dat[,i])))
  tab[tab$question_qno==i,"no_opinion"] <- length(which(dat[,i]==111))
}
...
pcq2_cc <- pcq2[,-which(names(pcq2) %in% c("q136", "unit_type", "research_id"))]
...
i <- which(names(pcq2) %in% c(no_opinion_scales, "unit_type", "research_id"))
pcq2_of <- pcq2[,-i]
...
Note: Exploratory Factor analysis is a close cousin to principal components analysis. Unlike PCA, which accounts for the variance in the items, EFA accounts for the correlations among them. EFA is not a data reduction method. The two approaches are similar but are used for different ends. EFA is used to identify latent variables, whereas PCA is a data reduction method.
...
retain <- pcq_lookup[which(pcq_lookup$include_comparative_psych_analysis=="yes"),]$question_qno
retain <- retain[which(retain %ni% paste0("q", c(1,7,8,9)))]
pcq <- pcq[,c(retain, "unit_type", "research_id")]
...
# Dataset 1: all items, complete cases ####
pcq2_w2_cc <- pcq2_w2[,-which(names(pcq2_w2) %in% c("q136", "unit_type", "research_id"))]
...
no_opinion_scales <- paste0("q", c(62:68,111:112,119:122,135:137,139:146))
pcq_lookup$no_opinion_scales <- ifelse(pcq_lookup$question_qno %in% no_opinion_scales, 1,0)
i <- which(names(pcq2_w2) %in% c(no_opinion_scales, "unit_type", "research_id"))
...
Incarcerated men on different types of units have different privileges and different access to activity. There is a general sense that climate differs on different types of unit. 'GP' units are a generic unit type, "GP-TC" holds individuals required to complete drug programming, and "HONS" units have enhanced privileges because of good behavior. We would like to find out if clustering patterns in the data align with unit type. We will use data set 2 from wave 2 (N=320), which contains all scales without no opinion options and has imputed missing values. We first calculate scale scores based on the factors identified in the exploratory data analysis. 
...
Next, identify the number of clusters in the data containing scale means. Reasonable minds could differ on where the break in the scree plot is. We go with the first two clusters for ease of interpretation. We then plot these clusters by unit type. We use two different pairs of scales to do so. First, we use the staff and prisoner relationships scales. Second, we use the Autonomy and Availability of Activities scales. The two clusters identified appear to slice the plot down the diagnonal. Interestingly, individuals in cluster 1 have relatively high scores on staff and prisoner relationships while having relatively low scores on the autonomy and availability of activity scales, and vice versa for individuals in cluster 2. There appear to be individuals in each group on each unit type. It is hard, however, to see a clear division by unit type in the plots.
