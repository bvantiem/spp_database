...
ph_lookup_all <- pcq_lookup[pcq_lookup$scale %in% c("safety", "unsafety", "conduct", "violence", "sleep", "health", "care", "discrimination"),]
ph_lookup_all <- ph_lookup_all[ph_lookup_all$question_no %ni% c(33,34,40,41),]
...
for(i in 1:length(questions.list)){ 
  ...
  # Subset data 
  question_ch <- ph_lookup_all[ph_lookup_all$question_no %in% questions,]$question
  ph_lookup <- ph_lookup_all[ph_lookup_all$question_no %in% questions,]
...
read.csv("data/admin/pcq_wave1_participation.xlsx")
library("excel.link")

df <- xl.read.file("data/admin/pcq_wave1_participation.xlsx", password = "chester")
...
for(i in 1:length(questions.list)){ 
  ...
  questions <- questions.list[[i]]
  # Subset data 
  question_ch <- pcq_lookup[pcq_lookup$question_no %in% questions,]$question
  ph_lookup <- pcq_lookup[pcq_lookup$question_no %in% questions,]
  qnos <- ph_lookup$question_qno
  pcq_temp <- pcq2[,c("research_id", "block", "unit_type", qnos)]
